...
const Dashboard = () => {
  ...
  const stats = [
    ...
    },
    {
      ...
      value: new Set(forms.map(f => f.audience))?.size?.toString() || "0",
      trend: null,
      trendValue: null
    },
    {
      ...
      value: new Set(forms.map(f => f.language))?.size?.toString() || "0",
      trend: null,
      trendValue: null
    }
  ];
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#0A0F1E] bg-dot-grid text-white">
        ...
        <main className="pt-16">
          <div className="max-w-9xl mx-auto px-4 md:px-6 lg:px-8 py-6 md:py-8 lg:py-10">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6 md:mb-8">
              <div>
                <h1 className="font-heading font-bold text-2xl md:text-3xl lg:text-4xl text-white mb-1.5 tracking-tight">
                  Dashboard
                </h1>
              ...
              <Button
                ...
                className="w-full lg:w-auto font-semibold shadow-lg shadow-indigo-600/30"
              >
                Create New Form
              </Button>
            </div>
            ...
            {filteredForms?.length === 0 && forms?.length > 0 ? (
              <div className="bg-[#111827] rounded-xl shadow-xl p-8 md:p-12 text-center border border-[#1F2937]">
                <div className="w-16 h-16 md:w-20 md:h-20 bg-white/5 border border-white/10 rounded-full flex items-center justify-center mx-auto mb-4 text-gray-400">
        ...
        {copiedId && (
          <div className="fixed bottom-4 right-4 md:bottom-6 md:right-6 bg-emerald-600 text-white px-4 py-3 md:px-6 md:py-4 rounded-xl shadow-2xl border border-emerald-500 flex items-center gap-2.5 animate-slide-in z-200">
            <svg className="w-5 h-5 md:w-6 md:h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
            </svg>
            <span className="text-sm md:text-base font-semibold">Link copied to clipboard!</span>
          </div>
        )}
      </div>
    </>
