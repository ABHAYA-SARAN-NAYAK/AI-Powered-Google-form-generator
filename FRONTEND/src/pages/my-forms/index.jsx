...
import { FORMS_CHANGED_EVENT } from '../../utils/formsStorage';
...
const MyForms = () => {
  ...
  const SortButton = ({ columnKey, label }) => (
    <button
      ...
      className="flex items-center gap-1.5 hover:text-white transition-smooth uppercase tracking-wider text-xs font-bold text-gray-400"
    >
      <span>{label}</span>
      ...
      <main className="pt-16">
        <div className="max-w-9xl mx-auto px-4 lg:px-6 py-6 lg:py-8">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6 lg:mb-8">
            <div>
              <h1 className="font-heading text-2xl lg:text-3xl font-bold text-white mb-1.5 tracking-tight">
                My Forms
              </h1>
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-1 bg-[#111827] border border-[#1F2937] rounded-xl p-1 shadow-md">
                <button
                  onClick={() => setViewMode('table')}
                  className={`p-2 rounded-lg transition-smooth ${
                    viewMode === 'table' ? 'bg-primary text-white shadow-md' : 'text-gray-400 hover:text-white hover:bg-white/5'
                  }`}
                ...
                <button
                  ...
                  className={`p-2 rounded-lg transition-smooth ${
                    viewMode === 'grid' ? 'bg-primary text-white shadow-md' : 'text-gray-400 hover:text-white hover:bg-white/5'
                  }`}
          ...
          {filteredAndSortedForms?.length === 0 ? (
            ...
            <>
              {viewMode === 'table' ? (
                <div className="bg-[#111827] rounded-xl border border-[#1F2937] shadow-xl overflow-hidden">
                  <div className="overflow-x-auto scrollbar-custom">
                    <table className="w-full text-left">
                      <thead className="bg-[#0A0F1E]/80 border-b border-[#1F2937]">
                        <tr>
                          <th className="px-4 py-3.5 lg:px-6 lg:py-4 w-12">
                            <input
                              type="checkbox"
                          ...
                          <th className="px-4 py-3.5 lg:px-6 lg:py-4">
                            <SortButton columnKey="title" label="Form Title" />
                          </th>
                          <th className="px-4 py-3.5 lg:px-6 lg:py-4 hidden md:table-cell">
                            <SortButton columnKey="type" label="Type" />
                          </th>
                          <th className="px-4 py-3.5 lg:px-6 lg:py-4 hidden lg:table-cell">
                            <SortButton columnKey="createdDate" label="Created" />
                          </th>
                          <th className="px-4 py-3.5 lg:px-6 lg:py-4 hidden lg:table-cell">
                            <SortButton columnKey="audience" label="Audience" />
                          </th>
                          <th className="px-4 py-3.5 lg:px-6 lg:py-4 hidden xl:table-cell">
                            <SortButton columnKey="language" label="Language" />
                          </th>
                          <th className="px-4 py-3.5 lg:px-6 lg:py-4 hidden md:table-cell">
                            <SortButton columnKey="status" label="Status" />
                          </th>
                          <th className="px-4 py-3.5 lg:px-6 lg:py-4 text-right uppercase tracking-wider text-xs font-bold text-gray-400">
                            Actions
                          </th>
                        </tr>
                      </thead>
          ...
          {showCopyNotification && (
            <div className="fixed bottom-6 right-6 z-100 bg-emerald-600 text-white px-5 py-3.5 rounded-xl shadow-2xl border border-emerald-500 flex items-center gap-2.5 animate-slide-in">
          ...
          {toast?.message && (
            <div className="fixed bottom-6 left-4 right-4 md:left-auto md:right-6 z-100 animate-slide-in">
              <div
                className={
                  `rounded-xl shadow-2xl px-5 py-3.5 flex items-center gap-3 max-w-sm border ` +
              ...
              >
                <Icon name={toast.variant === 'success' ? 'CheckCircle2' : toast.variant === 'error' ? 'AlertCircle' : 'Info'} size={18} />
