...
-- Analytics cache: stores aggregated response data to reduce Google API calls
create table if not exists public.form_analytics (
  id         uuid        primary key default gen_random_uuid(),
  form_id    text        not null unique,          -- Google Form ID (string)
  analytics_json jsonb   not null default '{}',
  updated_at timestamptz not null default now()
);

create index if not exists form_analytics_form_id_idx on public.form_analytics (form_id);
create index if not exists form_analytics_updated_at_idx on public.form_analytics (updated_at);
