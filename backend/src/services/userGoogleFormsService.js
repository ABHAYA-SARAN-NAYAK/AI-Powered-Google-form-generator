...
function toImageItemRequest(question, index, { qrImageUrl } = {}) {
  const imageUrl = qrImageUrl || env.CANTEEN_QR_URL || '';
  if (!imageUrl) return null; // Skip image items if no URL available

  return {
    createItem: {
      location: { index },
      item: {
        title: sanitizeDisplayText(question.title || 'Scan QR Code to Pay'),
        imageItem: {
          image: {
            sourceUri: imageUrl,
            properties: {
...
function toGoogleItemRequest(question, index, { isQuiz } = {}) {
  const normalizedQ = sanitizeFileUploadQuestion(question);
  const title = normalizedQ.title;
  const required = !!normalizedQ.required;
  ...
  const base = {
    createItem: {
      ...
      item: {
        ...
        ...(normalizedQ.description ? { description: normalizedQ.description } : {}),
        questionItem: {
          question: {
            required
          }
  ...
  switch (String(normalizedQ.type || question.type || '').toLowerCase()) {
    case 'short_text':
    case 'short_answer':
...
export async function createGoogleFormFromSpecForUser({ userId, spec, formType, qrImageUrl } = {}) {
  if (!userId) {
  ...
  try {
    ...
    // If a QR image URL is provided, inject it as the first item (index 0)
    if (qrImageUrl) {
      requests.push({
        createItem: {
          item: {
      ...
      insertIndex = 1; // Questions start after the QR image
    }

    for (const q of questions) {
      // Skip any image-type items Gemini might still output
