...
function toImageItemRequest(question, index, { qrImageUrl } = {}) {
  ...
  return {
    createItem: {
      location: { index },
      item: {
        title: question.title || 'Scan QR Code to Pay',
        imageItem: {
          image: {
            sourceUri: imageUrl,
            properties: {
...
function sanitizeFileUploadQuestion(q) {
  if (!q) return q;

  const typeStr = String(q.type || '').toLowerCase();
  const titleStr = String(q.title || q.question || '').toLowerCase();
...
function toGoogleItemRequest(question, index, { isQuiz, qrImageUrl } = {}) {
  if (question.type === 'image') {
    return toImageItemRequest(question, index, { qrImageUrl });
  }
...
export async function createGoogleFormFromSpec(spec, { formType, qrImageUrl } = {}) {
  ...
  for (let i = 0; i < questions.length; i += 1) {
    ...
    requests.push(toGoogleItemRequest(q, insertIndex, { isQuiz, qrImageUrl }));
    insertIndex += 1;
  }
